...
export class CallManager extends EventEmitter {
  ...
  private async routeIncoming(conn: Connection, env: Envelope): Promise<void> {
    ...
    switch (env.type) {
      ...
      case "LIST_SESSIONS": {
        ...
        if (process.env.PEERD_DEBUG_SUBS) if (process.env.PEERD_DEBUG) console.log(`[cm] LIST_SESSIONS inbound; have ${available.length} available`);
      ...
      case "LIST_SESSIONS_RESPONSE": {
        ...
        if (process.env.PEERD_DEBUG_SUBS) if (process.env.PEERD_DEBUG) console.log(`[cm] LIST_SESSIONS_RESPONSE for request ${p.request_id}: ${p.sessions.length} sessions`);
  ...
  async listRemoteSessions(peerName: string, timeoutMs: number = 5000): Promise<SessionInfo[]> {
    ...
    if (process.env.PEERD_DEBUG_SUBS) if (process.env.PEERD_DEBUG) console.log(`[cm] listRemoteSessions: sending LIST_SESSIONS request_id=${request_id} to ${peerName}`);
  ...
  private async handleIncomingShareFile(conn: Connection, cid: string, env: Envelope): Promise<void> {
    if (process.env.PEERD_DEBUG) console.log(`[cm] inbound SHARE_FILE call=${cid} from=${env.from} seq=${env.seq}`);
    const call = this.calls.get(cid);
    if (!call) {
      if (process.env.PEERD_DEBUG) console.log(`[cm] SHARE_FILE: unknown call ${cid}; have calls=[${Array.from(this.calls.keys()).join(",")}]`);
      conn.send(errorEnvelope(this.selfName, ErrorCode.UNKNOWN_CALL, "no such call", { call_id: cid }));
      return;
    }
    if (call.state !== "CONNECTED") {
      if (process.env.PEERD_DEBUG) console.log(`[cm] SHARE_FILE: call ${cid} not CONNECTED (state=${call.state})`);
      conn.send(errorEnvelope(this.selfName, ErrorCode.UNKNOWN_CALL, `call in state ${call.state}`, { call_id: cid }));
      return;
    }
    const senderRole = env.from === call.caller ? "caller" : "callee";
    if (call.floor !== senderRole) {
      if (process.env.PEERD_DEBUG) console.log(`[cm] SHARE_FILE: OUT_OF_TURN (floor=${call.floor}, sender=${senderRole})`);
      conn.send(errorEnvelope(this.selfName, ErrorCode.OUT_OF_TURN, "floor mismatch", { call_id: cid, in_response_to_seq: env.seq }));
      return;
    }
    await this.appendTranscript(call, "in", env);
    ...
    if (process.env.PEERD_DEBUG) console.log(`[cm] emitting file_shared message event for call ${cid}`);
    this.emit("message", evt);
  }
